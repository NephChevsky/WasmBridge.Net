...
If you (or your IDE) can't use VS Code's `blazorwasm` launch config generated by `init --vscode`,
`debug` is an IDE-agnostic alternative:

```
npx wasmbridge-net debug
```

It syncs a Debug build, starts your dev server (`--dev-command`, default `"npm run dev"`), and
launches Chrome/Edge with remote debugging enabled (`--remote-debugging-port`, default `9222`) so
*any* debugger can attach - `chrome://inspect`, another IDE's Blazor WebAssembly/attach-to-browser
support, or VS Code's own launch config pointed at the same port. `--browser edge` switches
browsers, and `--browser-path <path>` overrides auto-detection of the browser executable if it
isn't installed at its default location.

Note: the C#-breakpoint bridging (translating the browser's V8 debug protocol to the .NET/mono
debugger) is still provided by whichever IDE/debugger you attach - `debug` only automates the
build/serve/launch steps that are otherwise identical across IDEs.
