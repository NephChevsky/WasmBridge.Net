...
`blazorwasm` debug type) in the enclosing workspace - the nearest ancestor directory with a
`.sln`/`.slnx` (or `.git`) file, found by walking up from your front-end's directory:

```
npx wasmbridge-net init --vscode
```

An existing task/launch config with the same label/name is left untouched (pass `--force` to
overwrite it). Use `--port <n>` (default `5173`) if your dev server doesn't run on Vite's default
port, and `--browser chrome|edge` (default `chrome`) to pick the debug browser.
